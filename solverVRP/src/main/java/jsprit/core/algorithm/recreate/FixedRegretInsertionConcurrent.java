...
import jsprit.core.algorithm.recreate.FixedRegretInsertion.DefaultScorer;
import jsprit.core.algorithm.recreate.FixedRegretInsertion.ScoredJob;
import jsprit.core.algorithm.recreate.FixedRegretInsertion.ScoringFunction;
...
*
*/
public class FixedRegretInsertionConcurrent extends AbstractInsertionStrategy {


    private static Logger logger = LogManager.getLogger(FixedRegretInsertionConcurrent.class);
	...
	public FixedRegretInsertionConcurrent(JobInsertionCostsCalculator jobInsertionCalculator, VehicleRoutingProblem vehicleRoutingProblem, ExecutorService executorService) {
    ...
    private ScoredJob nextJob(final Collection<VehicleRoute> routes, List<Job> unassignedJobList, List<Job> badJobList) {
        ...
        for (final Job unassignedJob : unassignedJobList) {
            completionService.submit(new Callable<ScoredJob>() {
                ...
                public ScoredJob call() throws Exception {
                    return FixedRegretInsertion.getScoredJob(routes, unassignedJob, insertionCostsCalculator, scoringFunction);
                }
        ...
        try{
            for(int i=0; i < unassignedJobList.size(); i++){
                ...
                if(sJob instanceof FixedRegretInsertion.BadJob){
